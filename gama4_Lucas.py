...
while running:
    ...
    if not ticks_to_bonus:
        bonus = env.addParticles(1,y = 0, colour = (0,200,0))
        ticks_to_bonus = 720
    else:
        ticks_to_bonus -= 1
    ####################Queda dos bônus-END####################
    ...
    if not ticks_to_bomb:
        bomb = env.addParticles(1,y = 0, colour = (60,50,50))
        ticks_to_bomb = 1200
    else:
        ticks_to_bomb -= 1
    ####################Queda das bombas####################    
    ...
    text = GAME_HP_font.render("HP: {0}".format(hp), 1, (0, 255, 0))
    screen.blit(text, (width - 90, 10))
    if (hp == False): #movimento das particulas entre outras coisas que precisam ser atualizadas
