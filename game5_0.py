...
def play_sound(path):
  ...
  sound.play()
class Arena(PyParticles0.Environment):
    pass


screen = pygame.display.set_mode((width, height))
env = Arena(width, height)
scenery = pygame.image.load(os.path.join("scenery0.jpg"))
scenery_theta = 0
#env.addParticles(5,y = 0)
#Music----------------------
pygame.mixer.music.load('main0.mp3')
...
while running:
    ...
    if not ticks_to_particles:
        ticks_to_particles = 360
        env.addParticles(5,y = 0, damage = 1)
    else:
        ticks_to_particles -= 2
    ###################Queda dos meteorso-END##################

    ######################Queda dos bônus######################
    if not ticks_to_bonus:
        bonus = env.addParticles(1,y = 0, colour = (0,200,0), life = 1, message = '+1 HP', label = 'HP', sound = 'life.wav')
        ticks_to_bonus = 720
    else:
        ticks_to_bonus -= 1
    ####################Queda dos bônus-END####################
    ...
    if not ticks_to_bomb:
        bomb = env.addParticles(1,y = 0, colour = (60,50,50),message = 'KABOOM !',label = 'TNT', sound = 'destroyed.wav')
        ticks_to_bomb = 480
    else:
        ticks_to_bomb -= 1
    ####################Queda das bombas####################    
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if (bullet.x ==x0 and bullet.y ==y0):
                ...
                bullet.vel = PyParticles0.aim(x0,y0,xf,yf)
    ...
    screen.blit(scenery, (-400 - scenery_x, -100 - scenery_y))
    text = GAME_HP_font.render("HP: {0}".format(env.hp), 1, (0, 255, 0))
    screen.blit(text, (width - 90, 10))
    if (env.hp <= 0): #movimento das particulas entre outras coisas que precisam ser atualizadas
    ...
    screen.blit(score, ( (width - score_width)/2,height-160))
    screen.blit(rank,  ( (width - rank_width)/2,height-190))
